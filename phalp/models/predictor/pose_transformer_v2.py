...
class Pose_transformer_v2(nn.Module):
    ...
    def extract_2d_and_3d_keypoints(self,final_visuals_dic):
        # iterate through each frame
        for frame in final_visuals_dic.keys():
            # batch_size is basically number of people in a frame
            batch_size=len(final_visuals_dic[frame]['track_ids'])
            prediction_pose=final_visuals_dic[frame]['prediction_pose']
            ...
            for per_pose in prediction_pose:
                pred_smpl_params['global_orient'].append(torch.tensor(per_pose[0:9]))
                pred_smpl_params['body_pose'].append(torch.tensor(per_pose[9:216]))
                pred_smpl_params['betas'].append(torch.tensor(per_pose[216:226]))
                pred_cam.append(torch.tensor(per_pose[226:229]))

            pred_smpl_params['global_orient']=torch.stack(pred_smpl_params['global_orient'])
            pred_smpl_params['body_pose']=torch.stack(pred_smpl_params['body_pose'])
            pred_smpl_params['betas']=torch.stack(pred_smpl_params['betas'])
            pred_cam=torch.stack(pred_cam)
            ...
            pred_keypoints_2d = self.perspective_projection(pred_keypoints_3d,
                                                       translation=pred_cam_t,
                                                       focal_length=focal_length / self.cfg.MODEL.IMAGE_SIZE)
            output['pred_keypoints_2d'] = pred_keypoints_2d.reshape(batch_size, -1, 2)

            final_visuals_dic[frame]['pred_keypoints_3d']=output['pred_keypoints_3d']
            final_visuals_dic[frame]['pred_keypoints_2d']=output['pred_keypoints_2d']
